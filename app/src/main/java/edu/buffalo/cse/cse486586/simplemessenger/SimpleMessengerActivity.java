...
public class SimpleMessengerActivity extends Activity {
    ...
    static final String REMOTE_PORT0 = "11108"; //avd0
    static final String REMOTE_PORT1 = "11112"; //avd1
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG,"AVD listens on port:"+myPort);
...
/*
*   Asynchronously read data. Not really required in PA1 since there is only one sending device
*
                    class MyRunnable implements Runnable {
                        ...
                        public void run() {
                            try {
                                ...
                                br.close(); //since we're reading only one line at a time, just close the stream
                                socket.close();
                ...
                new Thread(new MyRunnable(clientSocket)).start();*/
                    ...
                    Log.d(TAG,"accept() from"+clientSocket.getRemoteSocketAddress().toString());

                    BufferedReader br = new BufferedReader(new InputStreamReader(clientSocket.getInputStream(),"UTF-8"));
                    publishProgress(br.readLine());
                    br.close(); //since we're reading only one line at a time, just close the stream
                    clientSocket.close();
                    Log.d(TAG,"free to accept");
                    Log.w(TAG,"clientSocket closed:"+clientSocket.isClosed());//isconnected only tells me whether i have correctly closed the socket or not.
                }
            } catch (IOException e) {
                e.printStackTrace();
            } catch (Exception e) {
    ...
    private class ClientTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... msgs) {
            try {
                ...
                Log.w(TAG,"I'm listening on Virtual Port "+myPort+" and connectig to "+remotePort);
                Socket socket = new Socket(InetAddress.getByAddress(new byte[]{10, 0, 2, 2}),
                        Integer.parseInt(remotePort));
                String msgToSend = msgs[0];

                BufferedWriter bw=new BufferedWriter(new OutputStreamWriter(socket.getOutputStream(),"UTF-8"));
                bw.write(msgToSend);    // OR write(msgToSend,0,msgToSend.length());
                bw.flush();
                //Make sure that the other end has actually read the data. Prematurely closing the socket will cause a loss in data. The only way to know whether the remote party closed the socket is
                //to read() the stream. When a read returns -1 we know for sure that the other end has closed the stream. See ServerTask - we close the stream fitst there(after reading), and THEN we
